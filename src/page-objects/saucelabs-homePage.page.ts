...
class SaucelabsHomePage {
    ...
    //readonly itemButtonRemove : Locator;
   // readonly itemButtonAdd : Locator;
    ...
    constructor(page : Page){
     ...
     //this.itemButtonRemove = page.getByTestId('inventory-list').getByTestId('inventory-item').filter({hasText:'Remove' });
     this.itemsList = page.getByTestId('inventory-list').getByTestId('inventory-item');
    // this.itemButtonAdd = this.itemsList.getByTestId('inventory-item').filter({hasText:'Add to cart' });
    ...
    async validateCheckoutIcon(): Promise<void>{
    ...
    async clickonRandomItem(): Promise<void>{
        ...
        const itemButtonAdd = randomItem.getByRole('button').filter({hasText:'Add to cart' });
        const itemButtonRemove = randomItem.getByRole('button').filter({hasText:'Remove' });
        ...
        const selecteditemText = (await randomItem.getByTestId('inventory-item-name').textContent())!;
    ...
    async validateHamburguerMenu(): Promise<void> {
        await expect(this.hamburguerMenu).toBeVisible();
    }

    async validateHompage(): Promise<void>{
